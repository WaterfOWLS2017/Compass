...
class HMC5883L:
    ...
    def __init__(self):
        self.port = wiringpi.wiringPiI2CSetup(0x1E)
    ...
    def read(self):
        ...
        X = self.port.wiringPiI2CReadReg16(0x03)
        Z = self.port.wiringPiI2CReadReg16(0x05)
        Y = self.port.wiringPiI2CReadReg16(0x07)
        return (X, Y, Z)
    ...
    def isLocked(self):
        ...
        '''
        return bool(self.port.wiringPiI2CReadReg8(0x09) & 0x02)
        
    def isReady(self):
        '''Checks if new magnetic data is available in the data registers

        This goes low as soonas one data register is read, and won't go high until
        all registers are read (lock is gone) and a new measurement has again been
        made.
        '''
        return bool(self.port.wiringPiI2CReadReg8(0x09) & 0x01)
    ...
    def setRate(self, rate):
        ...
        old = self.port.wiringPiI2CReadReg8(0x00)
        self.port.wiringPiI2CWriteReg8(0x00, (old & 0xE3) | (self.data_out_rate[rate] << 2))

    def getRate(self):
        '''Gets current data output rate configuration from magnetometer'''
        return self.data_out_rate_inv[(self.port.wiringPiI2CReadReg8(0x00) & 0x1C) >> 2]
    ...
    def setSamplesAveraged(self, avg):
        ...
        old = self.port.wiringPiI2CReadReg8(0x00)
        self.port.wiringPiI2CWriteReg8(0x00, (old & 0x9F) | (self.sample_average[avg] << 5))

    def getSamplesAveraged(self):
        '''Gets current average sample per measurement configuration from magnetometer'''
        return self.sample_average_inv[(self.port.wiringPiI2CReadReg8(0x00) & 0x1C) >> 5]

    def getGainSetting(self):
        '''Gets current gain setting from magnetometer'''
        return self.gain_setting_inv[(self.port.wiringPiI2CReadReg8(0x01) & 0xE0) >> 5]
    ...
    def setGainSetting(self, gain):
        ...
        old = self.port.wiringPiI2CReadReg8(0x01)
        self.port.wiringPiI2CWriteReg8(0x01, (old & 0x1F) | (self.gain_setting[gain] << 5))
    ...
    def getMeasurementMode(self):
        ...
        '''
        return self.measurement_mode_inv[(self.port.wiringPiI2CReadReg8(0x02) & 3)]
    ...
    def setMeasurementMode(self, mode):
        ...
        old = self.port.wiringPiI2CReadReg8(0x02)
        self.port.wiringPiI2CWriteReg8(0x02, (old & 0xFC) | (self.measurement_mode[mode]))
